  ...
  abstract class ApiContext[T <: BaseController: ClassTag] extends BaseContext {
    ...
    class Response[D: Reads](
      ...
      /**
        ...
        * @return A Specs2 match result.
        */
      def validate(detailsPath: List[String]): MatchResult[D] = {
        val json = contentAsString(result)
        status(result) must equalTo(httpStatus)
        contentType(result) must beSome(MimeTypes.JSON)
        ...
        val parsedJson = Json.parse(json)
        val parsedDetails = detailsPath.foldLeft(parsedJson \ "details")((acc, path) => acc \ path)
    ...
    object Response {
      ...
      def apply[D: Reads](
        ...
        new Response(httpStatus, result, code, description, details).validate(detailsPath)
